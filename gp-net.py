...
def main():
    ...
    parser.add_argument(
        "-cutoff", "--cutoff", help="MEGNet radial cutoff. [default: 5]", type=int
    )
    parser.add_argument(
        "-width", "--width", help="MEGNet gaussian width. [default: 0.5]", type=float
    )
    parser.add_argument(
        "-prev",
        action="store_true",
    ...
    parser.add_argument(
        "-rate", help="Adam optimizer Learning rate. [default: 0.01]", type=float
    )
    parser.add_argument(
        "-amp", help="Amplitude of the GP kernel. [default: 1.0]", type=float
    )
    parser.add_argument(
        "-length", help="The length scale of the GP kernel. [default: 1.0]", type=float
    )
    parser.add_argument(
        "-maxiters",
    ...
    if args.noactive:
        ...
        assert (fraction[0] + fraction[1]) == 1.0, "The sum of -frac must be 1!"
        ...
        assert stop < 1.0, "Stop argument should be less than 1!"
        assert (
            nsplit == 1
        ...
        if args.noactive:
            if not args.nomeg:
                (
                    ...
                    prop, bond, nfeat_global, cutoff, width, args.include, fraction
                )
            ...
            if nsplit == 1:
                ...
                OptAmp_fold = numpy.array([])
                OptLength_fold = numpy.array([])
                Optmae_val_fold = numpy.array([])
                Optmse_val_fold = numpy.array([])
                mae_test_fold = numpy.array([])
                ...
                for fold, (train_idx, val_idx) in enumerate(kf.split(Xpool)):
                    ...
                    OptAmp_fold = numpy.append(OptAmp_fold, amp)
                    OptLength_fold = numpy.append(OptLength_fold, length_scale)
                    Optmae_val_fold = numpy.append(Optmae_val_fold, Optmae_val)
                    Optmse_val_fold = numpy.append(Optmse_val_fold, Optmse_val)
                    mae_test_fold = numpy.append(mae_test_fold, mae_test)
            ...
            if args.repeat:
                ...
                Optmae_val_cycle = numpy.array([])
                mae_test_cycle = numpy.array([])
                mse_test_cycle = numpy.array([])
                sae_test_cycle = numpy.array([])
                ...
                for i in range(max_query + 1):
                    ...
                    training_data = numpy.append(training_data, len(ytrain))
                    Optmae_val_cycle = numpy.append(Optmae_val_cycle, Optmae_val)
                    mae_test_cycle = numpy.append(mae_test_cycle, mae_test)
                    mse_test_cycle = numpy.append(mse_test_cycle, mse_test)
                    sae_test_cycle = numpy.append(sae_test_cycle, sae_test)
                ...
                numpy.save("%s/ytrain.npy" % datadir, ytrain)
                numpy.save("%s/yval.npy" % datadir, yval)
                ...
                latent_train = numpy.load("%s/latent_train.npy" % datadir)
                latent_test = numpy.load("%s/latent_test.npy" % datadir)
                latent_val = numpy.load("%s/latent_val.npy" % datadir)
                ...
                for i in range(max_query + 1):
                    ...
                    if i < max_query:
                        ...
                        samp_idx = numpy.append(samp_idx, idx)

                logging.info("Writing the results to file ...")
                numpy.save("%s/training_data_for_plotting.npy" % datadir, training_data)
                numpy.save("%s/gp_mae.npy" % datadir, mae_test_cycle)
                numpy.save("%s/gp_mse.npy" % datadir, mse_test_cycle)
                numpy.save("%s/gp_sae.npy" % datadir, sae_test_cycle)
                numpy.save("%s/samp_indices.npy" % datadir, samp_idx)
                numpy.save("%s/Xtest.npy" % datadir, numpy.delete(Xtest, samp_idx))
                if maxiters > 0:
                    numpy.save("%s/val_mae.npy" % datadir, Optmae_val_cycle)
