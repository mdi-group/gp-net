...
class adam:
    ...
    def train_test_split(datadir, prop, tsne_pool, tsne_test, ypool_dft,
        ...
        if maxiters > 0:
            np.save(file="%s/OptLoss.npy" %datadir, OptLoss) 
            np.save(file="%s/OptAmp.npy" %datadir, OptAmp) 
            np.save(file="%s/OptLength.npy" %datadir, OptLength) 
            np.save(file="%s/Optmae.npy" %datadir, Optmae) 
            np.save(file="%s/Optmse.npy" %datadir, Optmse)
            np.save(file="%s/Optsae.npy" %datadir, Optsae)
        np.save(file="%s/ypool.npy" %datadir, ypool_dft.numpy())
        np.save(file="%s/ytest.npy" %datadir, ytest_dft.numpy())
        np.save(file="%s/gp_mean.npy" %datadir, gprm_dft.mean().numpy())
        np.save(file="%s/gp_stddev.npy" %datadir, gprm_dft.stddev().numpy())
        np.save(file="%s/gp_variance.npy" %datadir, gprm_dft.variance().numpy())
    ...
    def k_fold(datadir, prop, tsne_train, tsne_val, tsne_test, ytrain_dft,
        ...
        if maxiters > 0:
            ...
            np.save(file="%s/OptAmp.npy" %datadir, OptAmp)
            np.save(file="%s/OptLength.npy" %datadir, OptLength)
            np.save(file="%s/Optmae_val.npy" %datadir, Optmae_val)
            np.save(file="%s/Optmse_val.npy" %datadir, Optmse_val)
            np.save(file="%s/Optsae_val.npy" %datadir, Optsae_val)
        np.save(file="%s/ytrain.npy" %datadir, ytrain_dft.numpy())
        np.save(file="%s/yval.npy" %datadir, yval_dft.numpy())
        np.save(file="%s/ytest.npy" %datadir, ytest_dft.numpy())
        np.save(file="%s/mae_test.npy" %datadir, mae_test)
        np.save(file="%s/mse_test.npy" %datadir, mse_test)
        np.save(file="%s/sae_test.npy" %datadir, sae_test)
        np.save(file="%s/gp_mean.npy" %datadir, gprm_dft.mean().numpy())
        np.save(file="%s/gp_stddev.npy" %datadir, gprm_dft.stddev().numpy())
        np.save(file="%s/gp_variance.npy" %datadir, gprm_dft.variance().numpy())
