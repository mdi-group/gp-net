...
def ReadData(datafile, ZeroVals):
    ...
    if ZeroVals == False:
        ...
        mask = np.array([i for i, val in enumerate(inputs[prop]) if abs(val) == 0.0])
...
def megnet_input(prop, bond, nfeat_global, cutoff, width, keep_zeroes=False, *fraction):
    ...
    if len(fraction) == 1:
        if (fraction[0][0] + fraction[0][1]) == 1.0:
            ...
            x_pool = np.array(valid_structures[0:pool_boundary])
            y_pool = np.array(valid_targets[0:pool_boundary])
            x_test = np.array(valid_structures[pool_boundary:])
            ...
            y_pool = np.array(valid_targets[0:pool_boundary])
            x_test = np.array(valid_structures[pool_boundary:])
            y_test = np.array(valid_targets[pool_boundary:])
